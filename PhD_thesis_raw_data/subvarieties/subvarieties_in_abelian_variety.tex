...
\[\begin{tikzcd}[column sep=10mm,row sep=-2mm]
	{\gamma_C:} &[-12mm] C & {C'} & [5mm] {\mathbb{P}^{n-1}} & [-12mm]{=} &[-12mm] {\hspace{-10mm}\mathbb{P}\!\left( H^0(\omega_{C'} \otimes \eta)^* \right)} \\
	&&&& \cong & {\Gr\left(n-1, H^0(\omega_{C'} \otimes \eta)\right)} \\[1mm]
	& {\tilde{p}} & p &&& {H^0\left(\omega_{C'} \otimes \eta(-p)\right)}
...
which implies that $C'$ is hyperelliptic. By Fact ???, this forces $C$ to be hyperelliptic as well, and hence it lies outside the scope of our discussion.

Unfortunately, \eqref{eq:remove_extra_C} does not hold in general. A counterexample, communicated to me by Prof. Farkas, will be presented in Proposition ???.
