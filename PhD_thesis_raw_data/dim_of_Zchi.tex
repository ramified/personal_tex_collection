...
\begin{equation*}
\begin{aligned}
  \;& \left(a^{(m)}_* \alpha \right) \cup \beta = a^{(m)}_* \left( a^{(m),*} \alpha \cup \beta \right) = \left( \prod_{i=1}^{k} m_i^{2-d_i}\right) e_{\beta}\; c_{\{1,\ldots,2g\}} \\ 
...
\begin{equation*}
\begin{aligned}
  \;& \left(a^{(m)}_* \sigma(\alpha) \right) \cup \beta = a^{(m)}_* \left( a^{(m),*} \sigma(\alpha) \cup \beta \right) = \left( \prod_{i=1}^{k} m_{\sigma(i)}^{2-d_i}\right) e_{\beta}\; c_{\{1,\ldots,2g\}} \\ 
...
\begin{aligned}
  f_{ij}(m_1,\ldots,m_{2g-2}) = \frac{e_{ij}}{2^{g-1}} \sum_{\sigma \in S_{2g-2}} \prod_{l=1}^{g} \left( m_{\sigma(2l-1)}-m_{\sigma(2l)} \right)^2
\end{aligned}
\end{equation}
...
\end{aligned}
\end{equation*}
However, we still cannot determine $[a^{-1}(K)] \in \Hnm^{2g-2}(C^{2g-2}; \mathbb{Z})$ explicitly. However, we can still not determine $[a^{-1}(K)]$ explicitly. The choice of $a_i$ and $b_i$ might affect how $[a^{-1}(K)]$ is expressed. 
...
\item Extend this approach to the $E_6$ case. (We can discuss about this next time)
\end{enumerate}
\endgroup
\end{ques}
