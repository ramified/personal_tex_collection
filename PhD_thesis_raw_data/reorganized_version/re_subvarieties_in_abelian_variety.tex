...
In this subsection, we reinterpret $\gamma_Z$ using a functorial and more transparent framework. This permits to define the conormal Gauss map and its monodromy group for any morphism $\phi_Z: Z \to \Gr(r,n)$ with $\dim Z = r$, and clarifies the relation between the monodromy group and $\deg \phi_Z$.
...
Examples of subvarieties of abelian varieties can be obtained in two ways: one may fix an abelian variety and consider cycles within it, or begin with a variety $Z$ and construct a map to an abelian variety, such as the Albanese map. In the latter perspective, the setting changes slightly: $X$ is a smooth projective variety of dimension $r$, $\iota_X: X \to A$ is a morphism to an abelian variety $A$ of dimension $n$, and $Z := \iota_X(X)$ denotes its image in $A$.

In this subsection, we begin with the Albanese morphism $X \to \Alb(X)$, from which we derive some basic properties of the tangent Gauss map. Any other morphism to an abelian variety factors over the Albanese morphism, and then analogous methods apply, see Proposition \ref{prop:tangent_Gauss_outside_Albanese}.
...
$(2)$: The implication “$\Rightarrow$” follows from the geometry of the projection $\pr_1: X \times X^{[2]} \to X$.
...
\begin{itemize}
\item If $C'$ is bielliptic and $\eta = \pr^* \eta_0$, where $\pr: C' \to E'$ is a bielliptic map and $\eta_0 \in \Pic^0(E')[2]$, then the tangent Gauss map $\phi_C: C \to \mathbb{P}^{n-1}$ has degree $4$, and the curve $C \subset A$ is invariant under a $2$-torsion translation in $A$.
\item Otherwise, the tangent Gauss map $\phi_C: C \to \mathbb{P}^{n-1}$ is a double cover onto its image, and $\Gal(\gamma_C)$ is big.
\end{itemize}
\end{proposition}
...
\end{tikzcd}\]
where $|\omega_{C'} \otimes \eta|$ denotes the Prym–canonical map. According to \cite[Corollary 2.2]{Cil20}, one has $\deg |\omega_{C'} \otimes \eta| \in \{1, 2\}$, and the case $\deg |\omega_{C'} \otimes \eta| = 2$ occurs precisely when $C'$ is bielliptic and $\eta = \pr^* \eta_0$. In this situation, let $h_E \colon E \to E'$ denote the unramified double covering corresponding to $\eta_0$, and let $\iota \in \Gal(E/E')$ be the associated involution. The following diagram is Cartesian:
...
In the process of persuing proving Conjecture \ref{conj:big_mon_for_curve_2}, we propose the following lema which will roll out a lot of trivial cases.
