...
$$\mathrm{II}_p: T_pC \times T_pC \longrightarrow N_pC.$$
...
\[\begin{tikzcd}[ampersand replacement=\&,column sep=10mm,row sep=7mm]
	...
	\arrow["{\mathrm{II}_p}", from=1-2, to=1-3]
	\arrow["\cong", from=1-2, to=2-2]
	\arrow["\cong", from=1-3, to=2-3]
	\arrow["{\times }"{description}, draw=none, from=2-1, to=2-2]
	\arrow["{\mathrm{II}_{\sigma(p)}}", from=2-2, to=2-3]
\end{tikzcd}\]
Upon identifying $T_pC$ with $T_{\sigma(p)}C$ via parallel transport, one obtains the relation $$\mathrm{II}_p = c(p) \cdot \mathrm{II}_{\sigma(p)}^2$$.
...
$$\kappa(p)=c \cdot \Nm(\mathrm{II}_p).$$
\end{lemma}
...
\end{proposition}
\begin{proof}
By Lemma \ref{lem:curv_of_curve_in_A}, we see that $\kappa(p)=|c(p)|^2 \cdot \kappa(\sigma(p))$. By Proposition \ref{prop:twisting_is_alg}, 
\begin{equation*}
\begin{aligned}
