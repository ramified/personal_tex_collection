...
Consider a non-degenerate subvariety $Z \subset A$ and a tuple $(m) = (m_1, \ldots, m_d) \in \mathbb{Z}^d$. Let $Z^{(m)} \subset A$ be as in Definition \ref{def:subvar_Z^{(m)}}. Denote $d = \deg \gamma_Z$, $\widetilde{d} = d/2$, and set $c_l := c_{c,l}(\Lambda_Z)$ to be the Chern--Mather class of $Z$, with $*$ representing the Pontryagin product.
\begingroup
\upshape
...
\begin{equation*}
\begin{aligned}
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{c_Z^{(m)}} \sum_{\lambda \dashv l} \;\mu_d^{\lambda} \left( \bigast_{i=1}^r c_{\lambda_i}\right) \\ 
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{c_Z^{(m)}} \sum_{\lambda \dashv l} \;\widetilde{\mu}_{\widetilde{d}}^{\lambda} \left( \bigast_{i=1}^r c_{\lambda_i}\right) \\
\end{aligned}
\end{equation*}
...
Moreover, for $\textbf{l}=(l_1,\ldots,l_k) \in \mathbb{N}_{\geqslant 0}^k$ with $\sum l_i=l$, we set
$$\alpha(m)^{2\textbf{l}}:=  \left( \sum_{i \in A_1}m_i \right)^{2l_1} \cdots \left( \sum_{i \in A_k}m_i \right)^{2l_k}$$
...
\begin{aligned}
  ...
  =\;& \sum_{\alpha}\; \mu(\hat{0},\alpha)   \sum_{l=0}^{n-1}\sum_{\sum l_i=l} \alpha(m)^{2\textbf{l}} \left(\bigast_{i=1}^k c_{l_i} \otimes H^l\right) \\  
  =\;& \sum_{l=0}^{n-1} \left(\sum_{\alpha} \sum_{\sum l_i=l}  \left(\mu(\hat{0},\alpha)    \alpha(m)^{2\textbf{l}} \bigast_{i=1}^k c_{l_i}\right)\right) \otimes H^l \\  
\end{aligned}
\end{equation*}
Therefore,
\begin{equation}\label{eq:CMclass_sym}
c_l\left(\Lambda_{Z^{(m)}}\right)=\frac{1}{c_Z^{(m)}}\sum_{\alpha} \sum_{\sum l_i=l}  \left(\mu(\hat{0},\alpha)    \alpha(m)^{2\textbf{l}} \bigast_{i=1}^k c_{l_i}\right)
\end{equation}
...
$$c_l\left(\Lambda_{Z^{(m)}}\right) \in \Hnm^{2l}(A)[m_1,\ldots,m_d]^{S_d}$$
...
\begin{aligned}
  ...
  \beta^{\eta}(\widetilde{m})^{2\textbf{l}}:=\;&  \left(\sum_{i \in A_1} \eta(i)m_i\right)^{2l_1} \cdots \left(\sum_{i \in A_k} \eta(i)m_i\right)^{2l_k} \\ 
  \beta(\widetilde{m}^2)^{\textbf{l}}:=\;&  \left(\sum_{i \in A_1} m_i^2\right)^{l_1} \cdots \left(\sum_{i \in A_k} m_i^2\right)^{l_k} \\   
\end{aligned}
\end{equation*}
where $\beta= \{A_1, \ldots, A_k\} \in \mathcal{P}(d/2)$ is a partition of length $k$, $\eta:[d/2] \longrightarrow \{ \pm 1\}$ is a sign function, and $\textbf{l}=(l_1,\ldots,l_k) \in \mathbb{N}_{\geqslant 0}^k$ with $\sum l_i=l$. Notice that 
$$\sum_{\eta}\beta^{\eta}(\widetilde{m})^{2\textbf{l}} = 2^{d/2}\,\beta(\widetilde{m}^2)^{\textbf{l}}.$$
...
\begin{aligned}
  ...
  =\;& \sum_{\beta,\eta}\; \frac{1}{2^k}\,\mu(\hat{0},\beta)   \sum_{l=0}^{n-1}\sum_{\sum l_i=l} \beta^{\eta}(\widetilde{m})^{2\textbf{l}} \left(\bigast_{i=1}^k c_{l_i} \otimes H^l\right) \\  
  =\;& \sum_{l=0}^{n-1} \left(\sum_{\beta,\eta} \sum_{\sum l_i=l}  \left( \frac{1}{2^k}\, \mu(\hat{0},\beta)    \beta^{\eta}(\widetilde{m})^{2\textbf{l}} \bigast_{i=1}^k c_{l_i}\right)\right) \otimes H^l \\  
  =\;& \sum_{l=0}^{n-1} \left(\sum_{\beta} \sum_{\sum l_i=l}  \left( 2^{d/2-k}\, \mu(\hat{0},\beta)    \beta(\widetilde{m}^2)^{\textbf{l}} \bigast_{i=1}^k c_{l_i}\right)\right) \otimes H^l \\ 
\end{aligned}
\end{equation*}

Therefore,
\begin{equation}\label{eq:CMclass_signsym}
c_l\left(\Lambda_{Z^{(m)}}\right)=\frac{1}{c_Z^{(m)}}\sum_{\beta} \sum_{\sum l_i=l}  \left( 2^{d/2-k}\, \mu(\hat{0},\beta)    \beta(\widetilde{m}^2)^{\textbf{l}} \bigast_{i=1}^k c_{l_i}\right)
\end{equation}
...
\begin{enumerate}[(1)]
\item  The Chern--Mather class
$$c_l\left(\Lambda_{Z^{(m)}}\right) \in \Hnm^{2l}(A)[m_1^2,\ldots,m_{d/2}^2]^{S_{d/2}}$$
...
\begin{equation*}
\begin{aligned}
  \mu_d^{\lambda}:=\;& \sum_{\alpha \in \mathcal{P}(d)} \sum_{\substack{\textbf{l} \dashv \lambda\\\textbf{l}:\text{ length }k}} \phantom{2^{\widetilde{d}-k}} \mu(\hat{0},\alpha)    \alpha(m)^{2\textbf{l}} d^{k-r} &&\in \mathbb{Z}[m_1,\ldots, m_d]^{S_d} \\ 
  \widetilde{\mu}_{\widetilde{d}}^{\lambda}:=\;& \sum_{\beta \in \mathcal{P}(\widetilde{d})}  \sum_{\substack{\textbf{l} \dashv \lambda\\\textbf{l}:\text{ length }k}} 2^{\widetilde{d}-k} \mu(\hat{0},\beta)    \beta(\widetilde{m}^2)^{\textbf{l}} (2\widetilde{d})^{k-r} &&\in \mathbb{Z}[m_1^2,\ldots, m_{\widetilde{d}}^2]^{S_{\widetilde{d}}} \\ 
\end{aligned}
\end{equation*}
In this definition:
\begin{itemize}
\item $\alpha=\{A_1,\ldots,A_k\} \in \mathcal{P}(d)$ is a set partition with $k = \#\alpha$ blocks (analogously for $\beta$);
\item $\textbf{l}=(l_1,\ldots,l_k) \in \mathbb{Z}^k$, and $\mathbf{l} \dashv \lambda$ indicates that $\mathbf{l}$ is of type $\lambda$, meaning that its nonzero components, counted with multiplicities, form the partition $\lambda$;
\item $\mu(\hat{0}, \alpha) = (-1)^{d-k} \prod_{i=1}^{k}\, \left(\rule{0mm}{3mm}|A_i|-1\right)$ is the Möbius function;
\item $\alpha(m)^{2\textbf{l}}=  \left( \sum_{i \in A_1}m_i \right)^{2l_1} \cdots \left( \sum_{i \in A_k}m_i \right)^{2l_k}$ and $ \beta(\widetilde{m}^2)^{\textbf{l}}=  \left(\sum_{i \in A_1} m_i^2\right)^{l_1} \cdots \left(\sum_{i \in A_k} m_i^2\right)^{l_k}$.
\end{itemize}
\end{definition}
