...
Consider a non-degenerate subvariety $Z \subset A$ and a tuple $(m) = (m_1, \ldots, m_d) \in \mathbb{Z}^d$. Let $Z^{(m)} \subset A$ be as in Definition \ref{def:subvar_Z^{(m)}}. Let $d = \deg \gamma_Z$, $\widetilde{d} = d/2$, and let $c_l := c_{M,l}(\Lambda_Z)$ be the Chern--Mather class of $Z$. Let $*$ be the Pontryagin product. We write $\lambda \dashv l$ to indicate that $\lambda=[\lambda_1,\ldots,\lambda_{k'}]$ is a partition of $l$.
\begingroup
\upshape
...
} Under this assumption, we set $Z = \iota_X(X)$ and $A = \Alb(Z)$. The corresponding Gauss map is then a rational map:
