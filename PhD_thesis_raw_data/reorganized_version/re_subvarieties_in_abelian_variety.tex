...
\subsection{Realized as characteristic cycles}
...
\section{Dimension and homology class}
...
\subsection{Reminder on the (homological) Chern--Mather class}
...
where $H_i \subseteq \mathbb{P}^\vee$ denotes a general linear subspace of dimension $i$. For brevity, we may later write $c_{M,i}(\Lambda)$ as $c_i(\Lambda)$, and $c_{M,i}(\Lambda_Z)$ simply as $c_i$.
\end{definition}
...
By Remark \ref{rem:CMclass_know_dim}, it suffices to consider the homology class $\mathbb{P}\Lambda_{Z^{(m)}}$. If we are not concerned with the scalar factor $c_Z^{(m)}$, we may equivalently compute $[\mathbb{P}\Lambda_{Z}^{(m)}]$, which coincides with the pushforward of $[\mathbb{P}\Lambda_{Z}^{\univ}]$. Consequently, the problem reduces to determining $[\mathbb{P}\Lambda_{Z}^{\univ}] \in \Hnm^{2dn}(A^d \times \mathbb{P}^{\vee})$.
...
\subsection{The homology class of $\mathbb{P}\Lambda_{Z}^{[d]}$}
...
\subsection{The homology class in type $C$ case}
...
denote the set of all perfect matchings of $[d]$. Each $\alpha \in \mathcal{P}2(d)$ induces an involution $\tau{\alpha}$ of $[d]$, and we define
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
where $\mu(\beta, \beta')$ denotes the Möbius function of the poset $\mathbb{P}(d/2)$. 
...
\begin{aligned}
  ...
  =\;& \sum_{\beta}\; \mu(\hat{0},\beta)  \left( a^{(\widetilde{m})},\Id_{\mathbb{P}^{\vee}}  \right)_* \Delta_{\beta,*}\! \left[ \mathbb{P}\Lambda_{Z}^{\widetilde{\geqslant \beta}} \right] \\ 
...
\begin{enumerate}[(1)]
\item  The Chern--Mather class
$$c_l\left(\Lambda_{Z^{(m)}}\right) \in \Hnm^{2l}(A)[m_1,\ldots,m_{d/2}]^{S_{d/2}}$$
is a polynomial of degree $2l$ with exponent at most $2r$, when expressed in the variables $m_1,\ldots,m_{d/2}$.
\item If $Z \subset A$ is a smooth curve, there exists a homogeneous symmetric polynomial $\widetilde{f}_{Z,l} \in \mathbb{Q}[m_1, \ldots, m_{d/2}]^{S_{d/2}}$ of degree $2l$ and exponent at most $2$, such that
$$c_l\left(\Lambda_{Z^{(m)}}\right)=\widetilde{f}_{Z,l}(m_1,\ldots,m_{d/2})  \left(\bigast_{i=1}^k c_{1}\right).$$
In this case, 
$$Z^{(m)} \subset A \text{ is a divisor } \quad\Longleftrightarrow\quad f_{Z,n-1}(m_1,\ldots,m_{d/2}) \neq 0.$$
\end{enumerate}
\end{corollary}
