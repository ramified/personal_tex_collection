...
\begin{fact}
Let $1^{k'} := [1, \ldots, 1] \dashv k'$. When $k' < \widetilde{d}$, we have
\begin{equation*}
\begin{aligned}
  \mu_d^{1^{k'}}=\;& \phantom{2^{\widetilde{d}}}\frac{1}{2^{k'}k'!} \sum_{\sigma \in S_{d}} \prod_{i=1}^{k'} \left( m_{\sigma(2i-1)}-m_{\sigma(2i)} \right)^2 \\
  \widetilde{\mu}_{\widetilde{d}}^{1^{k'}}=\;& 2^{\widetilde{d}}\frac{1}{2^{k'}k'!} \sum_{\sigma \in S_{d}} \prod_{i=1}^{k'} m_{\sigma(i)}^2 \\
  =\;& 2^{\widetilde{d}-k'}(\tilde{d}-k')! \sum_{1 \leqslant i_1 < \cdots l_{k'} \leqslant d} m_{i_1}^2\cdots m_{i_{k'}}^2 \\
\end{aligned}
\end{equation*}
