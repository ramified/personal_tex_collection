...
\DeclareMathOperator{\Csm}{C^{\operatorname{sm}}}
...
\begin{equation*}
\begin{aligned}
  c_{M,l}\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}} \sum_{\lambda \vdash l} \;\mu_d^{\lambda} \left( \bigast_{i=1}^{k'} c_{\lambda_i}\right) \\ 
\end{aligned}
\end{equation*}
for certain explicit coefficients $d_Z^{(m)} \in \mathbb{N}_{>0}$ and $\mu_d^{\lambda} \in \mathbb{Z}$.
\item When $Z=-Z$ and $\Gal(\gamma_{Z})=W(C_{d/2})$, the Chern--Mather classes of $Z^{(m)}$ can be written as
\begin{equation*}
\begin{aligned}
  c_{M,l}\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}} \sum_{\lambda \vdash l} \;\widetilde{\mu}_{\widetilde{d}}^{\lambda} \left( \bigast_{i=1}^{k'} c_{\lambda_i}\right) \\
\end{aligned}
\end{equation*}
for certain explicit coefficients $d_Z^{(m)} \in \mathbb{N}_{>0}$ and $\widetilde{\mu}_{\widetilde{d}}^{\lambda} \in \mathbb{Z}$.
\end{enumerate}
\endgroup
\end{theoremB}
...
Section \ref{sec:mon_examples} is devoted to the explicit computation of the monodromy group in selected cases, with a particular focus on the Prym curve situation. Proposition \ref{prop:Prym_unramified_double_cover} presents an example in which the monodromy group is small; however, this does not constitute a counterexample to Conjecture \ref{conj:big_mon_for_curve}, as the curve remains invariant under a non-trivial translation. In Subsection \ref{subsec:criteria_big_mon}, we collect several sufficient conditions for the monodromy group to be large, which we then employ in Subsection \ref{subsec:big_mon_case} to show that, for all remaining Prym curve cases with $g(C') \geqslant 4$, the monodromy group is necessarily large.
...
We work over the base field $\kappa = \mathbb{C}$, and by a variety we mean an integral separated scheme of finite type over $\mathbb{C}$. Let $A/\mathbb{C}$ be an abelian variety of dimension $n$, and let $Z \subseteq A$ be an irreducible closed subvariety of dimension $r$. We denote by $\iota_Z: Z \hookrightarrow A$ the inclusion morphism.
...
In this subsection, we reinterpret $\gamma_Z$ using a functorial and more transparent framework. This permits us to define the conormal Gauss map and its monodromy group for any morphism $\phi_Z: Z \longrightarrow \Gr(r,n)$ with $\dim Z = r$, and clarifies the relation between the monodromy group and $\deg \phi_Z$.

To simplify notation, we abbreviate $T_{0}A$ by $W$. Since each non-zero conormal vector $\xi  \in T^{*}_{0}A$ determines a hyperplane $H_{\xi} \in \Gr(n-1, T_0A)$, we have the isomorphisms 
\begin{equation*}
\begin{aligned}
...
$$T_p^* \alpha: T_{\alpha(p)}^* \Alb(X)= \Hnm^0(X,\Omega_X) \longrightarrow T_p^*X=\Hnm^0(X,\Omega_X|_p).$$
...
\end{aligned}
\end{equation*}
is the zero set of the section $\omega \in \Hnm^0(X,\Omega_X)$. The number $(-1)^r \deg \gamma_Z$ is the index (of the vector field) in the Poincaré--Hopf index formula (see \cite[p.~35]{Mil65}), and the monodromy group $\Gal(\gamma_Z)$ serves as a more refined invariant.
...
\begin{proof}
When $n = r$, the map $\phi_Z$ has a point as its target, so the equivalence is trivial. We shall thus restrict to the case $n > r$ in the subsequent discussion.
...
$$\left\{ S \in X^{[2]} \;\middle|\; h^0(X,\Omega_X \otimes \mathcal{I}_S)=n-r   \right\}$$
 under the map $\pi_X:X^{[2]} \longrightarrow X^{(2)} $ does not include the diagonal. Indeed, we can choose a section $s \in \Hnm^0(X,\Omega_X)$ that cuts out finitely many reduced points $p_1,\ldots,p_d$ in $X$. (Well, this may not be so true, since $\phi_Z$ is not always regular, such a section $s$ may vanish along some indeterminacy locus. However, for a general $s$, there will always be at least one isolated zero $p_1$, which is sufficient for our purposes.)
...
For “$\Leftarrow$”: as $\alpha$ is generically injective, one has $h^0(X, \Omega_X(-p)) = n-r$ for a general point $p \in X$. Therefore, for any subscheme $S$ containing such a general point, $h^0(X, \Omega_X \otimes \mathcal{I}_S) \geqslant n-r$.
\end{proof}
...
The lemma then follows directly from the semicontinuity theorem \cite[28.1.1]{vakil2017rising}.
\end{proof}
...
\begin{aligned}
  \;&  \phi_Z \text{ is not generically injective} \\ 
  \Longleftrightarrow\;\;& \text{For general $p \in Z$, there exists $q \neq p$ such that $\dim_{\mathbb{C}}\,\Hnm^0(Z,\Omega_Z(-p-q)) \cap V =n-r$} \\ 
  \Longleftrightarrow\;\;& \text{For all $p \in Z$, there exists $S \in Z^{[2]}$ such that $p\in S$ and $\dim_{\mathbb{C}}\,\Hnm^0(Z,\Omega_Z \otimes \mathcal{I}_S) \cap V=n-r$.} \\ 
\end{aligned}
\end{equation*}
\end{proposition}
...
A big monodromy group is any group of the following types:
{
...
makes $C$ an irreducible nondegenerate curve of degree $2g-2$. By Proposition \ref{prop:mon_of_embedded_curve} we get
...
\subsection{Extend automorphism from curve to abelian variety}
In the process of pursuing proving Conjecture \ref{conj:big_mon_for_curve_2}, we propose the following lemma which will rule out numerous trivial cases.
...
\begin{lemma}\label{lem:triv_twist_works}
 In Setting \ref{set:curve_map_to_abelian}, suppose that there exists a uniform number $c \in \mathbb{C}^*$ such that the following diagram commutes for every $p \in \tilde{C}$:
...
If $\tilde{\tau}^*$ acts by the scalar $\zeta_k$ on $T_0A$, then the induced action on $\Hnm_1(A;\mathbb{Q})$ has complex eigenvalues $\zeta_k$ and $\zeta_k^{-1}$, each occurring with multiplicity $n$. Hence, the characteristic polynomial of $\tilde{\tau}$ on $\Hnm_1(A;\mathbb{Q})$ is
 $$(T-\zeta_k)^n(T-\zeta_k^{-1})^n \in \mathbb{Q}[T].$$
...
\subsection{Conjecture in terms of curvature}
The twisting map may also be interpreted as comparing the second fundamental forms (and hence the curvature) at $p$ and $\sigma(p)$. Indeed, the differential of $\phi_C$ at $p$ induces
$$T_p\phi_C: T_pC \longrightarrow T_{\phi_C(p)} \Gr(1,n) = \Hom(T_pC, N_pC),$$
where $N_pC := T_pA / T_pC$ denotes the normal space at $p$. This map is equivalent to specifying the second fundamental form
$$\mathrm{I\!I}_p: T_pC \times T_pC \longrightarrow N_pC.$$
The construction is compatible with the action of $\sigma$ in the sense that the following diagram commutes:
% https://q.uiver.app/#q=
\[\begin{tikzcd}[ampersand replacement=\&,column sep=10mm,row sep=7mm]
	{T_pC } \&[-10mm] {T_pC} \& {N_pC} \\
	{T_{\sigma(p)}C} \& {T_{\sigma(p)}C} \& {N_{\sigma(p)}C}
	\arrow["{\times }"{description}, draw=none, from=1-1, to=1-2]
	\arrow["{T_p \sigma}"', from=1-1, to=2-1]
	\arrow["{\mathrm{I\!I}_p}", from=1-2, to=1-3]
	\arrow["\cong", from=1-2, to=2-2]
	\arrow["\cong", from=1-3, to=2-3]
	\arrow["{\times }"{description}, draw=none, from=2-1, to=2-2]
	\arrow["{\mathrm{I\!I}_{\sigma(p)}}", from=2-2, to=2-3]
\end{tikzcd}\]
Upon identifying $T_pC$ with $T_{\sigma(p)}C$ via parallel transport, one obtains the relation $$\mathrm{I\!I}_p = c(p) \cdot \mathrm{I\!I}_{\sigma(p)}.$$

From now on, we fix a Hermitian metric on $T_0A$, which induces a flat Hermitian metric on $A$. Its pullback along $C \subset A$ endows $\Csm$ with a Hermitian metric as well. For any point $p \in \Csm$, we denote by $\kappa(p)$ the Gauss curvature of $\Csm$ at $p$.

\begin{lemma}\label{lem:curv_of_curve_in_A}
The curve $\Csm$ is a minimal surface, and its Gauss curvature satisfies
$$\kappa(p)=-2\, (\Nm\mathrm{I\!I}_p)^2.$$
\end{lemma}
\begin{proof}
We begin by comparing the complex second fundamental form $\mathrm{I\!I}_p$ with its real counterpart
$$\mathrm{I\!I}_{p,\mathbb{R}}: T_pC_{\mathbb{R}} \times T_pC_{\mathbb{R}} \longrightarrow N_pC_{\mathbb{R}}.$$
Both admit a natural common extension to a $J$-bilinear form
$$\mathrm{I\!I}: T_pC \otimes_{\mathbb{R}} \mathbb{C} \times T_pC \otimes_{\mathbb{R}} \mathbb{C} \longrightarrow N_pC \otimes_{\mathbb{R}} \mathbb{C}$$
via the canonical identification
$$T_pC \otimes_{\mathbb{R}} \mathbb{C} \cong T_pC_{\mathbb{R}} \oplus T^{0,1}C.$$

Writing $\partial_{z} = \frac{1}{2} (\partial_{x}-i\partial_{y}) \in T_pC$, with orthonormal real tangent vectors $\partial_x, \partial_y \in T_pC_{\mathbb{R}}$, we obtain the following expressions for the mean curvature vector:
\begin{equation*}
\begin{aligned}
  \kappa_{\operatorname{mean}}(p)=\;&\frac{1}{2} \left(  \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}) + \mathrm{I\!I}(\partial_{y},\partial_{y}) \right)  \\ 
  =\;&\frac{1}{2} \left(  \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}) + \mathrm{I\!I}(J\partial_{x},J\partial_{x}) \right)  \\
  =\;&\frac{1}{2} \left(  \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}) - \mathrm{I\!I}(\partial_{x},\partial_{x}) \right)  \\ 
  =\;& 0
\end{aligned}
\end{equation*}
%https://math.stackexchange.com/questions/4031392/mean-curvature-vector
and for the Gauss curvature:
\begin{equation*}
\begin{aligned}
 \kappa(p) =\;& \left< \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}), \mathrm{I\!I}(\partial_{y},\partial_{y}) \right> - \left< \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{y}), \mathrm{I\!I}(\partial_{y},\partial_{x}) \right> \\ 
 =\;& \left< \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}), -\mathrm{I\!I}(\partial_{x},\partial_{x}) \right> - \left< \rule{0mm}{3.3mm} J\mathrm{I\!I}(\partial_{x},\partial_{x}), J\mathrm{I\!I}(\partial_{x},\partial_{x}) \right> \\ 
 =\;& -\left< \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}), \mathrm{I\!I}(\partial_{x},\partial_{x}) \right> - J \cdot (-J) \left< \rule{0mm}{3.3mm} \mathrm{I\!I}(\partial_{x},\partial_{x}), \mathrm{I\!I}(\partial_{x},\partial_{x}) \right> \\ 
 =\;& -2\, |\mathrm{I\!I}(\partial_{x},\partial_{x})|^2 \\
 =\;& -2\, (\Nm\mathrm{I\!I})^2 \\
 =\;& -2\, (\Nm\mathrm{I\!I}_p)^2 
\end{aligned}
\end{equation*}
\end{proof}

\begin{conjecture}\label{conj:curv}
Let $A$ be an abelian variety of dimension $n>2$. For a curve $C \subset A$ and a general point $p \in \Img \phi_C$, all points in $\phi_C^{-1}(p)$ exhibit the same Gauss curvature.
\end{conjecture}

\begin{proposition}\label{prop:constant_twisting_equiv_curv}
Conjecture \ref{conj:curv} is equivalent to Conjecture \ref{conj:const_twisting}.
\end{proposition}
\begin{proof}
By Lemma \ref{lem:curv_of_curve_in_A}, we obtain the relation $\kappa(p)=|c(p)|^2 \cdot \kappa(\sigma(p))$.\footnote{This identity may also be verified directly from the curvature formula expressed in terms of the Hermitian metric $h$:
$$\kappa = -\frac{1}{h} \partial_{z} \partial_{\bar{z}} \ln h.$$
The pullback metric $\sigma^* h$ satisfies
$$\sigma^* h(p)=|c(p)|^2 h(p),$$
so
\begin{equation*}
\begin{aligned}
 \kappa(\sigma(p)) =\;& -\frac{1}{\sigma^* h(p)} \left.\left( \rule{0mm}{3mm} \partial_{z} \partial_{\bar{z}} \ln \sigma^* h(z) \right)\right|_{z=p} \\ 
 =\;& -\frac{1}{|c(p)|^2 h(p)} \left.\left( \rule{0mm}{3mm} \partial_{z} \partial_{\bar{z}} \left(\ln h(z)+ \ln |c(z)|^2\right) \right)\right|_{z=p} \\ 
 =\;&\frac{1}{|c(p)|^2} \cdot -\frac{1}{ h(p)} \left.\left( \rule{0mm}{3mm} \partial_{z} \partial_{\bar{z}} \ln h(z) \right)\right|_{z=p}\\
 =\;&\frac{1}{|c(p)|^2} \cdot \kappa(p)
\end{aligned}
\end{equation*}
} By Proposition \ref{prop:twisting_is_alg}, 
\begin{equation*}
\begin{aligned}
& \text{$c$ is a constant map} \\
  \Longleftrightarrow\;& |c(p)| \equiv 1 \text{ for all $p$ } \\ 
  \Longleftrightarrow\;& \kappa(p)=\kappa(\sigma(p)) \text{ for all $p$ }
\end{aligned}
\end{equation*}
\end{proof}
...
\renewcommand\labelenumi{(\theenumi)}
\begin{enumerate}[(1)]
\item Suppose that $\mathbb{P}\Lambda_{Z_1}, \mathbb{P}\Lambda_{Z_2} \subset \mathbb{P}T^*A$ admit monodromy representations $$\rho_{\gamma_{Z_i}}: \pi_1(U,\xi_0) \longrightarrow \Aut\left(\gamma_{Z_i}^{-1}(\xi_0)\right),$$ 
...
\item When $Z_1=Z_2=Z$, we obtain a one-to-one correspondence:
$$\left\{ 
\begin{aligned}
...
\begin{lemma}
For $d=\deg \gamma_{Z}$, $\xi_0 \in \mathbb{P}T_0^* A$ a general point, write
\begin{equation*}
\begin{aligned}
...
\renewcommand\labelenumi{(\theenumi)}
\begin{enumerate}[(1)]
\item we obtain a one-to-one correspondence:
$$\left\{ 
\begin{aligned}
...
\Gal(\gamma_{Z})\text{-orbits of }  S_d
 \;\right\} 
 $$
 In the literature, $\Delta_d$ is usually called the big diagonal.
\end{enumerate}
\endgroup
\end{lemma}
...
$$\mathbb{P}\Lambda_{Z}^{(m)} = d_Z^{(m)}  \mathbb{P}\Lambda_{Z^{(m)}}$$
where $d_Z^{(m)} \in \mathbb{Z}_{>0}$ and $Z^{(m)} \subset A$ are uniquely determined. This gives rise to a family of subvarieties parametrized by $\mathbb{Z}^d$.
\end{definition}
...
\renewcommand\labelenumi{(\theenumi)}
\begin{enumerate}[(1)]
\item For all $g\in \Gal(\gamma_{Z})$, we have $ Z^{g(m)}=Z^{(m)}$, $d_Z^{g(m)}=d_Z^{(m)}$;
...
\begin{remark}
Suppose that $Z \subset A$ is smooth of general type. Then the characteristic cycle $\charcycle(\delta_Z)$ is irreducible and equals $\mathbb{P}\Lambda_Z$. In this situation, let $\lambda_1, \ldots, \lambda_d \in X^*(T_u)$ denote the weights corresponding to the points $p_1, \ldots, p_d \in \gamma_Z^{-1}(\xi_0)$.
...
By Remark \ref{rem:CMclass_know_dim}, it suffices to consider the homology class of $\mathbb{P}\Lambda_{Z^{(m)}}$. If we are not concerned with the scalar factor $d_Z^{(m)}$, we may equivalently compute $[\mathbb{P}\Lambda_{Z}^{(m)}]$, which by definition is the pushforward of $[\mathbb{P}\Lambda_{Z}^{\univ}]$. Consequently, the problem reduces to determining $[\mathbb{P}\Lambda_{Z}^{\univ}] \in \Hnm^{2dn}(A^d \times \mathbb{P}^{\vee})$.
...
where $\mu(\alpha, \alpha')$ denotes the Möbius function as defined in \cite[p.~141]{Ai12}.
...
which defines a homogeneous polynomial of degree $2l$ in $\mathbb{Z}[m_1,\ldots,m_d]$. 
\end{definition}
...
c_l\left(\Lambda_{Z^{(m)}}\right)=\frac{1}{d_Z^{(m)}}\sum_{\alpha} \sum_{\sum l_i=l}  \left(\mu(\hat{0},\alpha)    \alpha(m)^{2\boldsymbol{l}} \bigast_{i=1}^k c_{l_i}\right).
\end{equation}
...
$$c_l\left(\Lambda_{Z^{(m)}}\right)=f_{Z,l}(m_1,\ldots,m_d)  \left(\bigast_{i=1}^l c_{1}\right).$$
In this case, 
...
For $\beta \in \mathcal{P}(d/2)$, define
\begin{equation*}
\begin{aligned}
...
c_l\left(\Lambda_{Z^{(m)}}\right)=\frac{1}{d_Z^{(m)}}\sum_{\beta} \sum_{\sum l_i=l}  \left( 2^{d/2-k}\, \mu(\hat{0},\beta)    \beta(\widetilde{m}^2)^{\boldsymbol{l}} \bigast_{i=1}^k c_{l_i}\right).
\end{equation}
...
$$c_l\left(\Lambda_{Z^{(m)}}\right)=\widetilde{f}_{Z,l}(m_1^2,\ldots,m_{d/2}^2)  \left(\bigast_{i=1}^l c_{1}\right).$$
In this case, 
...
\begin{equation*}
\begin{aligned}
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}} \sum_{\lambda \vdash l} \;\mu_d^{\lambda} \left( \bigast_{i=1}^{k'} c_{\lambda_i}\right)\qquad && \text{type $A$ case} \\ 
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}} \sum_{\lambda \vdash l} \;\widetilde{\mu}_{\widetilde{d}}^{\lambda} \left( \bigast_{i=1}^{k'} c_{\lambda_i}\right)\qquad && \text{type $C$ case} \\
\end{aligned}
\end{equation*}
...
\begin{fact}
Let $1^{l} := [1, \ldots, 1] \vdash l$. When $l < \widetilde{d}$, we have
\begin{equation*}
\begin{aligned}
  ...
  =\;& 2^{\widetilde{d}-l}(\tilde{d}-l)! \sum_{1 \leqslant i_1 < \cdots < i_{l} \leqslant \widetilde{d}} m_{i_1}^2\cdots m_{i_{l}}^2 \\
\end{aligned}
\end{equation*}
...
Let $C$ be a smooth curve of genus $g$ embedded in its Jacobian $A := \Jac(C)$ via the Abel–Jacobi map $\AJ_C: C \hookrightarrow A$. In this case, writing $\Theta$ for the theta divisor of $A$, we have the formula \cite[p.~323]{BL04}
$$\bigast_{i=1}^{l} c_{1} = \frac{l!}{(g-l)!} \cdot \Theta^{g-l}.$$
When $C$ is non-hyperelliptic, we get
\begin{equation*}
\begin{aligned}
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}} \frac{1}{2^{l}(g-l)!} \sum_{\sigma \in S_{2g-2}} \prod_{i=1}^{l} \left( m_{\sigma(2i-1)}-m_{\sigma(2i)} \right)^2 \cdot \Theta^{g-l} \\ 
...
\begin{equation*}
\begin{aligned}
  c_l\left(\Lambda_{Z^{(m)}}\right)=\;& \frac{1}{d_Z^{(m)}}  \frac{2^{g-1-l} l!}{g-l}  \sum_{1 \leqslant i_1 < \cdots < i_{l} \leqslant g-1} m_{i_1}^2\cdots m_{i_{l}}^2 \cdot \Theta^{g-l} \\ 
