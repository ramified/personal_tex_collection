...
\begin{lemma}
Let $C'$ be a smooth projective curve of genus $g(C') \geqslant 2$, and let $\eta \in \Pic(C')$ be a line bundle of degree $s \geqslant 0$. Then the linear system $|\omega_{C'} \otimes \eta|$ is not generically injective if and only if one of the following holds:
\begin{enumerate}[(a)]
...
\end{enumerate}
Furthermore, in cases (a) and (b), the curve $C'$ must be hyperelliptic.
\end{lemma}
\begin{proof}
This is a consequence of Riemann--Roch. In case (a), since $\eta \in g^1_2$, the curve $C'$ is hyperelliptic. In case (b), one obtains a family of $g^1_3$'s of the form $\mathcal{O}_{C'}(x+y+u)$, which implies that $\mathcal{O}_{C'}(x+y) \in g^1_2$.
\end{proof}
...
Let $\phi_{\nu} = \phi_C \circ \nu$ denote the tangent Gauss map of $\tilde{C}$. Suppose that for there exists a uniform number $c \in \mathbb{C}^*$ such that the following diagram commutes for every $p \in \tilde{C}$:
