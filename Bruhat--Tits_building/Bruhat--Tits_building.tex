...
\only<1>{
  \begin{figure}
    ...
    \caption{$\mathcal{B}_{\SL_3(\mathbb{Q}_p)}$, from \href{https://www.math-berlin.de/images/stories/bruhat-tits.pdf}{a talk by Annette Werner}}
  \end{figure}
}
\only<2>{
  \begin{figure}
...
$T$ is a torus, so every rep decomposes as direct sum of $1$-dim reps.
\end{short}
}
\vspace{-5mm}
\begin{equation*}
...
\begin{equation*}
\begin{aligned}
  N_G(T)\phantom{/T}=\;& \left\{ \text{ monoidal matrices } \right\} \\ 
...
\begin{aligned}
  \left\{ \text{ Borel subgroups } \right\} =\;& \left\{ gBg^{-1} \ignore{\;\Big|\; g\in G} \right\} \hspace{7mm}&&\visible<5->{\cong G/B} \\ 
  \left\{ \text{ parabolic subgroups } \right\} =\;& \left\{ gPg^{-1} \ignore{\;\Big|\; g\in G} \right\} &&\visible<6->{\cong G/P} \\ 
  \left\{ \text{ maximal tori } \right\} =\;& \left\{ gTg^{-1} \ignore{\;\Big|\; g\in G} \right\} &&\visible<7->{\cong G/N_G(T)} \\ 
\end{aligned}
\end{equation*}
}
\end{fact}
...
\begin{equation*}
\begin{aligned}
  \left\{ \text{ Borel subgroups } \right\} =\;& \left\{ gBg^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/B \\ 
  \left\{ \text{ parabolic subgroups } \right\} =\;& \left\{ gPg^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/P \\ 
  \left\{ \text{ maximal tori } \right\} =\;& \left\{ gTg^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/N_G(T) \\ 
...
\begin{equation*}
\begin{aligned}
  \left\{ \text{ Borel subgroups } \right\} =\;& \left\{ gB_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/B_0 \\ 
  \left\{ \text{ parabolic subgroups } \right\} =\;& \left\{ gP_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/P_0 \\ 
  \left\{ \text{ maximal tori } \right\} =\;& \left\{ gT_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/N_G(T_0) \\ 
...
\end{tikzcd}\]  	
  \end{minipage}}
\visible<5->{  $$\left\{ \text{ (Weyl) chambers } \right\} \quad\stackrel{1:1}{\longleftrightarrow}\quad W \quad\stackrel{1:1}{\longleftrightarrow}\quad  \left\{ B \;\middle|\; B \supset T_0  \right\}$$}

\end{frame}

\begin{frame}[fragile]{Weyl group action on cocharacter lattices(revisited)}
\begin{short}
When $G=\SL_2(\kappa)$, $T= \left( \begin{smallmatrix}
a &  \\  & a^{-1} 
\end{smallmatrix} \right)$, $X_*(T)=\mathbb{Z}\varepsilon$, where
\vspace{-5mm}
  ...
  \begin{minipage}[t]{.55\textwidth}
    \vspace{0pt}
  	\centering
  	\only<1>{\includegraphics[height=18mm]{figure/SL_2_apartment_Weyl.png}
  	\vspace{-5mm}}
...
\vspace{-5mm}
  ...
  \begin{minipage}[t]{.55\textwidth}
    \vspace{0pt}
  	\centering
  	\only<1-3>{\includegraphics[height=35mm]{figure/SL_3_apartment_Weyl.png}}
  	\only<4>{\includegraphics[height=35mm]{figure/SL_3_apartment_Borel.png}}
  	\only<5->{\includegraphics[height=35mm]{figure/SL_3_apartment_parabolic.png}}
  \end{minipage}
  \begin{minipage}[t]{.43\textwidth}
  $\,$\\[-4mm]
% https://q.uiver.app/#q=
...
When $\kappa=\mathbb{F}_p$, $\mathcal{B}$ is finite. \ignore{i.e., having finite many chambers}}
\end{remark}}
\visible<3->{
\begin{proposition}
\begin{itemize}
\item Any two chambers lie in one apartment.
\visible<4->{\item There is a unique geodesic through any two points $p_1,p_2 \in \mathcal{B}$.}
\end{itemize}
}
\vspace{30mm}
\end{proposition}
...
$\mathcal{O}=\mathcal{O}_F$                          & ring of integers             & \visible<2->{$\mathbb{Z}_p$}                     \\
...
\begin{frame}[fragile]{standard subgroups in the  $p$-adic world}
...
\begin{aligned}
  ...
  \end{array}
  \right)
  }} \qquad&& \visible<6->{\text{Parahoric subgroup}} \\[-5mm]
  &&& \visible<6->{\text{\ignore{\hspace{-3mm}=Parabolic Iwahori subgroup}}}
\end{aligned}
\end{equation*}
\visible<7->{\begin{remark}
They also have moduli interpretations. For example,
\begin{equation*}
\begin{aligned}
  \GL_n(F)/I\cong\;& \left\{ \mathfrak{p}L=L_0 \subset L_1 \subset \cdots \subset L_n = L  \;\middle| \; L_{i+1}/L_i \cong \kappa  \right\}\\
...
\visible<2->{To get the Iwahori decomposition
...
\visible<3->{\begin{eg}
When $G=\GL_n(F)$,
$$W_{\ext}=\left\{ \text{ monoidal matrices } \right\} \Big/_{\setlength{\arraycolsep}{0pt}\renewcommand{\arraystretch}{0.8}\left(\begin{smallmatrix}
...
\begin{frame}[fragile]{Non-standard subgroups in the $p$-adic world}
\visible<2->{\begin{short}
Similarly,
\vspace{-4mm}
\begin{equation*}
\begin{aligned}
  \left\{ \text{ Iwahori subgroups } \right\} =\;& \left\{ gI_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/I_0 \\ 
  \left\{ \text{ paraholic subgroups } \right\} =\;& \left\{ g\widetilde{P}_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/\widetilde{P}_0 \\ 
  \left\{ \text{ maximal tori over $\mathcal{O}$ } \right\} =\;& \left\{ gT_0g^{-1}\ignore{\;\Big|\; g\in G} \right\} &&\cong G/N_G(T_0\ignore{(\mathcal{O})}) \\ 
...
\begin{frame}[fragile]{Extended Weyl group action(revisited)}
...
\visible<3->{
\begin{remark}
Similarly, any two chambers lie in one apartment,\\
and there is a unique geodesic through $p_1,p_2 \in \mathcal{B}$.
\end{remark}
}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The Gromov-Schoen theorem}
\begin{frame}{Plan of the talk}
\tableofcontents[currentsection,hideallsubsections]
\end{frame}

\begin{frame}[fragile]{The Gromov-Schoen theorem}
...
\visible<3->{For any reductive homomorphism
$$\rho: \pi_1(M) \longrightarrow \GL_n(F),$$
\visible<5->{there exists a $\pi_1(M)$-equivariant Lipschitz continuous regular harmonic map
$$h_{\rho}:\widetilde{M} \longrightarrow \mathcal{B}_{\GL_n(F)}$$
\vspace{-5mm}}}
\end{theorem}
\visible<4->{We call $\rho$ reductive when $\overline{\rho(\pi_1(M))}^{\Zar} \subseteq \GL_n(F)$ is reductive.
}
}
\end{frame}
...
\visible<2->{\begin{short}
  \begin{figure}
    \centering
    \only<1-2>{\includegraphics[width=.8\linewidth]{figure/regular_map_1.png}}
    \only<3>{\includegraphics[width=.8\linewidth]{figure/regular_map_2.png}}
	\only<4>{\includegraphics[width=.8\linewidth]{figure/regular_map_3.png}}
  \end{figure}
\end{short}
}
\end{eg}
