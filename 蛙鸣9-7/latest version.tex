...
2025年彩蛋更新：最近被复代数几何整破防了，在厚厚的工具书中见到了排山倒海的新结论，而四年前的自己写的科普显得如此稚嫩。故而今日在此记录下新见面的每一个Boss，希望有一天能战胜它们，看到它们之后的世界。

\begin{itemize}
\item \cite{Ar85I}：代数曲线的经典之作，几乎所有的结论都能在正文或习题中找到(能不意味着找到容易)。第一本书主要处理单个的曲线，到处充满了奇技淫巧；而第二本书\cite{Ar85II}处理模空间就友善多了(大概是因为我没怎么看吧)。

\item \cite{Barth04}:代数曲面的字典，第二版相较于第一版质量要高出许多，不过第一章冗长的预备知识容易让人弃坑。个人更欣赏\cite{beauville1996complex}的编排顺序，不知道有没有结合两者优势的代数曲面的书。

\item \cite{BL04}：阿贝尔簇的经典之作，常看常新。第一眼看目录觉得都是学过的东西，事实上每一节都在整新活(有时候直接整一个大定理并证了)。习题可能比\cite{Ar85I}善良一些，没有那么离谱的法式大题，但是数量也不少。

我们同时列出其他处理相同结论的教材供参考：
\begin{itemize}
\item \cite{Deb99}(基本搞定)：可以算是\cite{BL04}的前置，算是相当好的复阿贝尔簇的入门教科书了。
\item \cite{LR22}：\cite{BL04}的后置，主要处理各种Prym variety的细节，算是最新的科研成果总结了。整本书在速通\cite{BL04}的部分结论后就开始处理各种具体的例子。
\end{itemize}

\item \cite{Huy23}：超曲面的经典之作，特别是Cubic hypersurfaces.在Bonn的时候不了解Huybrechts的实力，读过他的书才体会到。这本书专注于例子，很多重量级结论和概念都是直接使用，导致对读者有很高的要求。我觉得大概是因为这些概念都在他的其他书中出现过了。

Huybrecht的书已经变成了一整个系列，这里按照时间顺序简要列出：
\begin{itemize}
\item \cite{Huymodulisheaves}:处理层的模空间。
\item \cite{HuyFMtrans}：处理Fourier-Mukai变换。
\item \cite{HuyK3}：处理K3曲面。
\end{itemize}
\item 还有一些我目前不打算看，但是可能相关的大部头：
\begin{itemize}
\item \cite{Vo02I,Vo02II}:专注于Hodge theory。这书我之前大致看了三分之一，看目录感觉基本都见过的概念。感觉这本书更专注于使用的工具，而我目前还是更缺乏深刻的例子。
\item \cite{Laz04I,Laz04II}:向量丛的正性。前摇过长，常常让我怀疑读它的意义(助眠神器)
\item \cite{GH94}:代数几何原理，足够厚导致啥边边角角的问题都可能在里面找到，但是往往是在书的中间，目录看着很乱，所以不知道该从何处开始看起。属于查字典时不会想到但是读过的人翻出来就很无语的书(好像是我没认真查文献)
...
打完这些Boss(看完这些书)，你应该就能手搓复代数簇了！用你庞大的几何对象库来和未知的数学战斗吧！
