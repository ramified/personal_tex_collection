...
\begin{document}
	\def\a{2}
	\def\aminusindex{0.985}
\begin{tikzpicture}
%[hyperbolic disc radius=\a cm,every to/.style={hyperbolic disc}]%use this to make lines the geodesics
\def\p{25}	
...
{\coordinate (A\y) at ($(\angle:\a*1.0)$);
\draw (\angle:\aminus) -- (\angle:\a);}

\foreach \y in {1,...,\q}
{
	\pgfmathtruncatemacro{\ytwo}{mod(\p*\y,\q)};
	\draw [color=black, very thin] (A\y) to (A\ytwo);	
}
